...
phyPVR<- function(x, phy = NULL, trait = NULL, envVar = NULL, method = "lamdba",
                     weights = NULL, scaled = FALSE, sig = TRUE, sig.t = 0.05, lamdba.t = 0.05, psr.t = 0.01, accvalue.t = 0.9, ...)
{
  ...
  if(method == "lamadba" ){
    ...
    reg <- data.frame(Vectors = 0, lambda.value = 0, p = 0)
    tmpTrait <- trait
    ...
    selection <- list(Method = "lamadba", Vectors = selected, Id = reg)
